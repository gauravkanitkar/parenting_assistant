...
export class SecureEnvLoader {
  ...
  static getOpenAIKey(): string {
    ...
    if (!encryptedKey || !masterKey) {
      ...
      if (!plainKey) {
        throw new Error('No OpenAI API key found. Please set OPENAI_API_KEY_ENCRYPTED + MASTER_KEY or OPENAI_API_KEY');
      }
      console.warn('⚠️  Using plain text API key. Consider encrypting it for better security.');
      return plainKey;
    }
